  ...
  <Listbox v-model="selectionOption">
    <div class="relative">
      <ListboxButton
        ...
        v-slot="{ open }"
      >
        {{ selectionOption ? SortByOptions.find((option) => option.value === selectionOption)?.name : 'Sort By' }}
        <GIcon
          :icon="mdiChevronDown"
          :class-name="cn(
            'w-5 h-5 ml-1.5 transition-transform duration-300 ease-in-out',
            {
...
const SortByOptions = [
  ...
  { value: SortableLables.Ibu, name: 'Ibu', unavailable: false },
]
const selectionOption = ref<SortableLables>($state.sortBy);
